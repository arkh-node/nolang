...
(defun belief-interval (w+ w- &optional (k *k*))
  "(w+,w-) → (values низ верх ширина). Вера как отрезок: положение И незнание.
   ...
   🔴 ЧТО ЭТОТ ОТРЕЗОК НЕ ЗНАЧИТ — И ПОЧЕМУ ОГОВОРКА СТОИТ ИМЕННО ЗДЕСЬ (06.08.2026, C1/C2).
   Отрезок НЕ несёт гарантии покрытия. Он говорит «столько ещё может изменить неизвестное»,
   а НЕ «истина лежит внутри с вероятностью 1−α». Это не осторожность формулировки, а
   устройство: границы получены из СЧЁТА свидетельств (imprecise Dirichlet model, Walley
   1996), а не из калибровки на размеченных данных. Conformal-калибровка ортогональна ему и
   не заменяет его — см. README, раздел Lineage.

   ⚠️ Оговорка эта была в README и ОТСУТСТВОВАЛА здесь — то есть её видел читающий текст и
   не видел читающий код. Найдено при обходе собственных формулировок; правило общее:
   **граница живёт там, где живёт величина**, а не только в витрине."
