...
(defun run-nolang (forms &key carrier)
  "→ (values склад журнал шагов непотреблённые-молчания отозванные источники).
   ...
   🔴 ИСТОЧНИКИ ВОЗВРАЩАЮТСЯ ШЕСТЫМ ЗНАЧЕНИЕМ (05.08.2026, B5). Они объявлены в прелюдии и
   живут внутри `with-prelude` — то есть умирают вместе с прогоном, как решётка и горизонт.
   Это правильно для вычисления и неудобно для вывода наружу: экспорт зовётся ПОСЛЕ, когда
   таблицы уже нет. Тот же дефект, что был с горизонтом (29.07): «отложенное вычисление в
   контексте, которого больше нет, — это не поздний ответ, а чужой». Поэтому источники не
   вычисляются позже, а отдаются вместе со складом."
  (with-prelude
   (let ((*errs* '()))
    (let* ((*corpus* (collect-corpus forms))
           (c (cfg (prefill-store *corpus* '()) '()
                  ...
                  '() '() forms))
           (n 0))
      ;; источники объявлены в прелюдии — регистрируем их до прогона, чтобы отдать наружу
      (dolist (f forms)
        (when (and (consp f) (string= (head-of f) "source")) (chk-source f)))
