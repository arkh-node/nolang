...
(defun chk-witness (form)
  ...
  (let* ((id (second form))
    ...
    (when (and g (not (g-shape-ok-p *lattice* g)))
      ...
      (setf g nil))
    ;; ── 🔴 ПОТОЛОК ПО ИСТОЧНИКУ (B3, 05.08.2026) ────────────────────────────────
    ;; До этого дня `:source` был только ключом дедупликации корней, а степень заявлял автор:
    ;; `witness x : machine_verified source my_imagination` типизировалось. Подъём степени
    ;; ВНУТРИ вывода был невыразим, ложь НА ВХОДЕ — выразима идеально. Пока `.nol` писал
    ;; человек, это условность; когда декларации пишет сам агент — дыра во всей конструкции.
    ;;
    ;; Теперь: если источник ОБЪЯВЛЕН в прелюдии, его класс — потолок. Заявленная степень
    ;; сходится с классом тем же `⊓`, что доказан в Agda (`g-meet`): выше потолка подняться
    ;; нельзя, ниже — можно (свидетель вправе признать себя слабее своего источника).
    ;; Необъявленный источник ничего не ограничивает — это переходное состояние, и оно
    ;; названо вслух в предупреждении, а не спрятано.
    (let ((srcs (kw form :source)))
      (dolist (sname srcs)
        (when (assoc sname *sources*)
          (multiple-value-bind (cls serr) (source-class sname)
            (cond
              ((eq serr :cycle)
               (err! :source id "свидетель ~a опирается на источник ~a с циклом в происхождении"
                     id sname))
              ((and cls g (not (g<= g cls)))
               ;; заявлено ВЫШЕ класса источника — не ошибка выполнения, а незаписываемое
               (err! :grade id
                     "свидетель ~a объявлен степенью ~a, но его источник ~a даёт не выше ~a. ~
                      Степень не поднимается происхождением: знание, пришедшее пересказом, ~
                      не становится первоисточником оттого, что так написано."
                     id (g-ru g) sname (g-ru cls))
               (setf g (g-meet g cls)))
              ((and cls g) (setf g (g-meet g cls)))
              ((and cls (null g)) (setf g cls)))))))
...
(defun chk-revoke (form)
  ...
  (let* ((who (second form))
    ...
    (dolist (f *done*)
      (when (consp f)
        (let ((h (head-of f)))
          ;; 🔴 Даватели берутся из ПРЕДЪЯВЛЕННЫХ прав (`permit`), а не из объявлений
          ;; действий: с 30.07 объявление несёт лишь требование «кто вправе», а дал ли
...
(defun chk-do (form)
  ...
  (let* ((an (second form)) (cn (third form))
    ...
    (cond
      ...
      (t
       ;; ── 🔴 ПРАВО ПРОВЕРЯЕТСЯ ДО ВЕРЫ И ДО СТЕПЕНИ ──────────────────────────────
       ;; Отсутствие права — не слабость основания, а ЗАПРЕТ: сколько свидетельств ни
       ;; принеси, разрешения они не заменят. Поэтому «никто не разрешал» есть ошибка
       ;; программы, а не свёрток гейта. Предъявление обязано стоять ДО действия: право,
       ;; данное задним числом, оправдывает уже совершённое — это не разрешение, а амнистия.
       (let* ((af (form-of-action an))
              (need (and af (kw af :needs-permission))))
         (when need
           (let ((дано (find-if (lambda (f)
                                  (and (consp f) (string= (head-of f) "permit")
                                       (eq (second f) an)))
                                *done*)))
             (unless дано
               (err! :право an
                     "~a требует разрешения от ~a, и оно не предъявлено ДО действия.~%  ~
                      Это не нехватка веры: право не набирается свидетельствами.~%  ~
                      Допишите ПЕРЕД действием:~%    ~
                      permit ~a \"<дословная цитата>\" from ~a at \"<где прочесть>\""
                     an need an need)))))
       ;; ── 🔴 ОБЯЗАТЕЛЬНОЕ СВИДЕТЕЛЬСТВО: ВЕТО, КОТОРОЕ НЕЛЬЗЯ ПЕРЕГОЛОСОВАТЬ ─────
       ;; `needs witness X` требует, чтобы X СУЩЕСТВОВАЛ и свидетельствовал ЗА (f > 0.5).
       ;; Это не масса веры: веру добирают числом свидетельств, а вето — нет. Красная
       ;; батарея остаётся красной, сколько бы голосов «за» ни принесли рядом, — и именно
       ;; поэтому проверка стоит в ТИПИЗАЦИИ, а не в гейте: «так нельзя», а не «мало веры».
       ;; Ровно половина (f = 0.5) не подтверждает: свидетель, разорванный пополам, не
       ;; говорит «да» — он говорит «не знаю», а вето снимается только утверждением.
       (let* ((af0 (form-of-action an))
              (must (and af0 (kw af0 :needs-witness))))
         (dolist (w must)
           (let ((b (look w)))
             (cond
               ((null b)
                (err! :mandatory w
                      "~a невозможно без свидетельства ~a, а его в программе НЕТ.~%  ~
                       Это не нехватка веры: требование поимённое. Отсутствие обязательного~%  ~
                       свидетельства есть отсутствие условия, а не слабое основание." an w))
               ((not (eq (bnd-kind b) :jud))
                (err! :mandatory w
                      "~a требует свидетельства ~a, но ~a — не свидетельство и не утверждение."
                      an w w))
               ((<= (bnd-f b) 0.5)
                (err! :mandatory w
                      "~a невозможно: обязательное свидетельство ~a НЕ подтверждает (f=~,3f).~%  ~
                       Это ВЕТО, а не порог: его нельзя перевесить другими свидетельствами,~%  ~
                       сколько бы их ни было. Чините то, о чём говорит ~a, — иначе действие~%  ~
                       не типизируется." an w (bnd-f b) w)))))) 
...
(assert-covers "chk-retract/перепрогон" '("lattice" "horizon" "import" "witness" "ask" "claim" "action")
               ;; permit пропущен по той же причине, что do/revoke: перепрогон при отзыве
               ;; свидетеля пересчитывает ВЕРУ, а право — другая ось, отзывом не задеваемая.
               :skip '("do" "retract" "revoke" "permit" "rule"))


;;; ── ПРЕДЪЯВЛЕНИЕ ПРАВА ──────────────────────────────────────────────────────
;;; 🔴 Право теперь имеет обе стороны в программе: `permit` даёт, `revoke` отзывает.
;;; Проверяется ровно одно — ФОРМА предъявления, а не смысл цитаты: язык не решает, значит
;;; ли фраза человека разрешение на это действие. Он требует, чтобы фраза была ДОСЛОВНОЙ,
;;; АДРЕСНОЙ (кто) и НАЗЫВАЛА МЕСТО (где прочесть). Судит человек, читающий вердикт.
(defun form-of-action (name)
  "Объявление действия NAME среди уже обработанных форм (оно из прелюдии и идёт раньше)."
  (find-if (lambda (f) (and (consp f) (string= (head-of f) "action") (eq (second f) name)))
           *done*))

(defun chk-permit (form)
  ;; (permit ACTION :quote "…" :who КТО :at "адрес")
  (let* ((a (second form))
         (who (kw form :who))
         (b (look a))
         (af (form-of-action a)))
    (cond
      ((null b) (err! :unknown a "разрешается неизвестное действие ~a" a))
      ((not (eq (bnd-kind b) :action)) (err! :slot a "~a — не действие" a))
      ((null af) (err! :unknown a "объявления действия ~a не найдено" a))
      (t
       (let ((need (kw af :needs-permission)))
         (cond
           ;; 🔴 Разрешение действию, которое его не требует, — не безобидная вежливость.
           ;; Оно создаёт видимость проверенного права там, где права никто не спрашивал,
           ;; и в вердикте читается как «человек это одобрил».
           ((null need)
            (err! :право a
                  "~a не требует разрешения, а оно предъявлено.~%  ~
                   Право спрашивают там, где действие влияет НАРУЖУ (`outward`).~%  ~
                   Либо действие объявлено не тем классом, либо разрешение здесь лишнее." a))
           ((not (eq need who))
            ;; чужая подпись под чужим правом: ровно то, против чего заведена адресность
            (err! :authority who
                  "~a вправе разрешить ~a, а разрешение дано от ~a.~%  ~
                   Право адресно: разрешает тот, кому оно принадлежит. Право распоряжаться~%  ~
                   чужим словом само есть право, и его тут никто не предъявлял."
                  need a who))))))))

;;; ── ОДНО ИМЯ — ОДНО ОБЪЯВЛЕНИЕ ─────────────────────────────────────────────
;;; 🔴 Повторное объявление МОЛЧА затирало предыдущее (предупреждение Тарантоги из карты
;;; передачи, подтверждено телом 30.07: два свидетеля с одним именем — первый исчезает
;;; бесследно, в складе остаётся второй). Это дыра в самом несущем: язык заведён, чтобы
;;; прослеживать ПРОИСХОЖДЕНИЕ, а тихая подмена делает происхождение неустановимым —
;;; сославшийся на имя не может знать, на что он сослался.
;;; Опаснее всего то, что затирание работает В ОБЕ СТОРОНЫ: им можно как испортить
;;; свидетельство, так и подменить неудобное удобным, — и в журнале не останется следа.
;;; 🔴 Способа «переобъявить намеренно» НЕТ и не будет: для пересмотра в языке есть отзыв
;;; (`retract` для свидетеля, `revoke` для права), и он ЗАПИСЫВАЕТСЯ в журнал. Разница
;;; ровно та, ради которой всё: пересмотр оставляет след, переписывание — нет.
(defparameter *declaring-forms* '("lattice" "import" "witness" "ask" "claim" "action")
  "Формы, вводящие ИМЯ. `rule` проверяется при разборе (раскрывается там же);
   `do`/`perform`/`retract`/`revoke`/`permit` имён не вводят — они ссылаются.")

(defun chk-unique (form seen)
  "Имя объявлено дважды → ошибка с указанием, чем было объявлено в первый раз."
  (let ((head (head-of form)) (name (second form)))
    (when (and (member head *declaring-forms* :test #'string=) (symbolp name) name)
      (let ((prev (gethash name seen)))
        (if prev
            (err! :redeclared name
                  "~a объявлено повторно (было: ~a, стало: ~a).~%  ~
                   Второе объявление МОЛЧА затирало первое — и сославшийся на это имя~%  ~
                   не мог знать, на что сослался.~%  ~
                   Переобъявления в языке нет: чтобы пересмотреть сказанное, есть отзыв~%  ~
                   (`retract` для свидетеля, `revoke` для права). Отзыв оставляет след~%  ~
                   в журнале, переписывание — нет. Дайте второму объявлению своё имя."
                  name prev head)
            (setf (gethash name seen) head))))))
