...
(let* ((forms (parse "witness a : strict says \"…\" source z evidence 8 for 1 against"))
       (w (first forms)))
  ;; 🔴 ФОРМА НЕСЁТ СЧЁТ, А НЕ КАРТУ НАД НИМ (правка 29.07, при вводе `horizon`).
  ;; Прежде парсер переводил счёт в `(f,c)` здесь же — и переводил с УМОЛЧАТЕЛЬНЫМ `k`,
  ;; потому что горизонт программы при разборе ещё не прочитан. Машина переводила обратно
  ;; с программным `k`, и вес раздувался. Теперь счёт отдаётся счётом; `(f,c)` считается
  ;; там, где нужно, и с тем `k`, который объявлен. Утверждение теста то же — проверяется
  ;; в правильном месте.
  (check "evidence 8 for 1 against → счёт отдан счётом: w⁺=8, w⁻=1"
         (and (< (abs (- (kw w :w+) 8.0)) 0.001) (< (abs (- (kw w :w- ) 1.0)) 0.001)))
  (check "…и (f,c) из этого счёта: f ≈ 0.889"
         (< (abs (- (nth-value 0 (evidence->fc (kw w :w+) (kw w :w-))) 0.8889)) 0.001))
  (check "…и c = 9/(9+1) = 0.9 ровно при горизонте 1"
         (< (abs (- (nth-value 1 (evidence->fc (kw w :w+) (kw w :w-))) 0.9)) 0.001)))
